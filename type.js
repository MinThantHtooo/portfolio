const nav = document.getElementById('navi').offsetHeight
document.documentElement.style.setProperty('--scroll--padding',nav - 1 + 'px');
  ...
  const startPieces = [
      ...
      function createBoard() {
          startPieces.forEach( (startPiece,i) => {
              ...
              if (i <= 15) {
                  square.firstChild.firstChild.classList.add('b')
              }
              if (i >= 48) {
                  square.firstChild.firstChild.classList.add('w')
              }
      ...
      function dragStart(e) {
          startP = e.target.parentNode.getAttribute('square-id');
      ...
      function dragDrop(e) {
          ...
          const existingPiece = targetSquare.querySelectorAll('.piece');
          const draggedPiece = draggedElement.firstChild;
          const draggedPieceColor = draggedPiece.classList.contains('w') ? 'white' : 'black';
  
          if (targetSquare.classList.contains('square') && targetSquare !== draggedElement.parentNode) {
          ...
          if (draggedPieceColor !== playerGo) {
              infoDisplay.textContent = "It's not your turn mate Go back";
              setTimeout(() => {
